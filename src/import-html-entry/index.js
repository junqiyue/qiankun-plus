...
function resolveCssUrls(cssContent, cssFileUrl) {
	// 获取 CSS 文件所在的目录
	const cssBaseUrl = cssFileUrl.substring(0, cssFileUrl.lastIndexOf('/') + 1);
	...
	// 替换相对路径为绝对路径
	const updatedCssContent = turn.replace(urlRegex, (match, quote, urlPath) => {
		// 忽略 data URI 和绝对路径
		if (/^(data:|https?:|\/)/i.test(urlPath)) {
			return match;
		}
		// 解析相对路径为绝对路径
		const absoluteUrl = new URL(urlPath, cssBaseUrl).href;
	...
	return getExternalStyleSheets(styles, fetch)
		.then(styleSheets => {
			embedHTML = styleSheets.reduce((html, styleSheet) => {
				...
				const styleSheetContent = resolveCssUrls(styleSheet.value, styleSrc);
